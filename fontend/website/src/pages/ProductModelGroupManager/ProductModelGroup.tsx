...
import type { CreateProductModelGroupDTO } from '@/types/productModelGroup';
import { useProductModelGroupHooks } from '@/hooks/productModelGroupHooks';
...
const initialFormData: CreateProductModelGroupDTO = {
    name: '',
    description: '',
    image: '',
    // categoryId: 0,
...
export const ProductModelGroupManagerScreen: React.FC = () => {
    const navigate = useNavigate();
    const [formData, setFormData] = useState<CreateProductModelGroupDTO>(initialFormData);
    ...
    const { useCreate, useGetList } = useProductModelGroupHooks();
    ...
    const { data: productModelGroups } = useGetList(1, '', 30);
    ...
    useEffect(() => {
        if (categoryId) {
            getFilteredBrandCategories({ page: 1, size: 5, categoryId })
        }
    }, [categoryId])
    ...
    return (
        ...
        >
            <Typography variant="h5" mb={2}>Sản phẩm cùng model</Typography>
            <Paper elevation={3} sx={{ p: 2 }}>
                <Stack direction={{ xs: 'column', md: 'row' }} spacing={2}>
                    {/* Cột trái */}
                    <Stack spacing={2} flex={2}>
                        <TextField
                            label="Tên model (Name)" name="name" required fullWidth size="small"
                ...
                {productModelGroups?.data?.items.map((item) => (
                    <Typography>
                        {item.name}
                    </Typography>
                ))}
            ...
            <Paper elevation={3} sx={{ p: 2 }}>
                <Stack direction={{ xs: 'column', md: 'row' }} spacing={1}>
                    ...
                    <Stack spacing={1} flex={2}>
                        <SelectCP
                            title="Bảo hành"
                            data={productModelGroups?.data?.items.map((item) => ({ label: item.name, value: item.id })) || []}
                            select={warrantyId}
                            onChange={(value) => {
                                setWarrantyId(value)
                                const temp = productModelGroups?.data?.items.find(item => item.id == value)
                                setModel(temp?.name || '')
                            }}
                        />
                ...
                >
                    ...
                    {/* {model.length && filteredProducts?.items.filter(element => element.code.includes(model)).map((item) => (
                        item.name.includes(model) ? <CheckBoxProductItem
                            product={item}
                            checked={listProductId.includes(item.id)}
                            onChange={handleSelectProduct}
                            attribute={handleAttribute(item.id)}
